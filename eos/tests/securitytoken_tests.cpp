...
class securitytoken_tester : public tester {
public:

   securitytoken_tester() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol), N(securitytoken) } );
      produce_blocks( 2 );

      set_code( N(securitytoken), contracts::token_wasm() );
      set_abi( N(securitytoken), contracts::token_abi().data() );

      produce_blocks();

      const auto& accnt = control->db().get<account_object,by_name>( N(securitytoken) );
      abi_def abi;
   ...
   fc::variant get_stats( const string& symbolname )
   {
      ...
      vector<char> data = get_row_by_account( N(securitytoken), symbol_code, N(stat), symbol_code );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(securitytoken), acc, N(accounts), symbol_code );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(securitytoken), N(create), mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(securitytoken_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, securitytoken_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, securitytoken_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, securitytoken_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, securitytoken_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, securitytoken_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, securitytoken_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( retire_tests, securitytoken_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, securitytoken_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( open_tests, securitytoken_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( close_tests, securitytoken_tester ) try {
